...
@WebMvcTest(FindByAgeAndNamePersonRestController.class)
class FindByAgeAndNamePersonRestControllerTest {
    ...
    void findByAge() throws Exception {
        ...
        // Given
        when(filterPersonService.findByNameAndAge("Alejandro", 34)).thenReturn(createPerson001().get());

        // When
        mvc.perform(get("/people/fullsearch").contentType(MediaType.APPLICATION_JSON)
                .param("name", "Alejandro").param("age", String.valueOf(34)))
                // Then
        ...
        verify(filterPersonService).findByNameAndAge("Alejandro", 34);
    }
}